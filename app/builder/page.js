...
export default function UniversalBuilder() {
  ...
  async function loadLatestProject() {
    try {
      ...
      if (data) {
        ...
        if (
          Array.isArray(data.build_plan) &&
          data.build_plan.length > 0
        ) {
          setPlan({
  ...
  async function startBuild() {
    ...
    try {
      ...
      const buildStages = Array.isArray(
      ...
      if (
        ...
        throw new Error(
          projectData?.error ||
            "Could not create the BOMBA project."
        );
      }
  ...
  async function buildNextStage() {
    ...
    setMessage("");
    setStageMessage(
      "🏗️ Starting the real build engine..."
    );
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data?.error ||
            "The build engine could not continue."
        );
      }
      ...
      >
        <div style={{ marginBottom: "26px" }}>
          ...
          >
            ...
            understand it, create a real plan, build it
            gradually, save your progress, and continue
