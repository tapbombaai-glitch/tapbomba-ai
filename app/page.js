...
return (  
  !containsHtml && (hasBuildInstruction || hasPlanSections)  
);
...
try {  
  ...
  if (html) {  
    ...
    setMessages((prev) => [  
      ...
      {  
        role: "assistant",  
        content:  
          "✅ App built successfully!\n\nYour live app preview is ready below. You can also copy or download the complete app.",  
      },  
    ]);  
  } else {  
    setMessages((prev) => [  
  ...
  setMessages((prev) => [  
    ...
    {  
      role: "assistant",  
      content: "⚠️ App build failed. Please try again.",  
    },  
  ]);  
} finally {  
  setBuilding(false);  
